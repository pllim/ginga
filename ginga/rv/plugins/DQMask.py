"""
DQ masking on an image.

**Plugin Type: Local**

``DQMask`` is a local plugin, which means it is associated with a
channel.  An instance can be opened for each channel.

**Usage**

This local plugin is used to display masks of the associated DQ array
of a given image.
...
from ginga.util.dqparser import DQParser

__all__ = ['DQMask']

# Generic DQ flags
_def_tab = """# TELESCOPE = GENERIC
# INSTRUMENT = GENERIC
DQFLAG SHORT_DESCRIPTION LONG_DESCRIPTION
0      "OK"              "Good pixel"
1      "BAD_1"           "Bad pixel of flag 1"
2      "BAD_2"           "Bad pixel of flag 2"
4      "BAD_4"           "Bad pixel of flag 4"
8      "BAD_8"           "Bad pixel of flag 8"
16     "BAD_16"          "Bad pixel of flag 16"
32     "BAD_32"          "Bad pixel of flag 32"
64     "BAD_64"          "Bad pixel of flag 64"
128    "BAD_128"         "Bad pixel of flag 128"
256    "BAD_256"         "Bad pixel of flag 256"
512    "BAD_512"         "Bad pixel of flag 512"
1024   "BAD_1024"        "Bad pixel of flag 1024"
2048   "BAD_2048"        "Bad pixel of flag 2048"
4096   "BAD_4096"        "Bad pixel of flag 4096"
8192   "BAD_8192"        "Bad pixel of flag 8192"
16384  "BAD_16384"       "Bad pixel of flag 16384"
32768  "BAD_32768"       "Bad pixel of flag 32768"
"""


class DQMask(LocalPlugin):

    def __init__(self, fv, fitsimage):
        # superclass defines some variables for us, like logger
        super(DQMask, self).__init__(fv, fitsimage)

        self.layertag = 'dqmask-canvas'
        self.pxdqtag = None

        self._cache_key = 'dq_by_flags'  # Ginga cannot use this anywhere else
        self._ndim = 2
        self._dummy_value = 0
        self._text_label = 'DQMask'
        self._text_label_offset = 4

        # TODO: Could be improved and expanded? Make it static by flag?
        self.imdqcolors = self.settings.get(
            'imdqcolors', ['blue', 'magenta', 'green'])
        self.n_color = len(self.imdqcolors)

        # TODO: Need to implement GUI to load mask file + ext
        self._def_parser = DQParser(_def_tab)

        # User preferences
        prefs = self.fv.get_preferences()
        self.settings = prefs.create_category('plugin_DQMask')
    ...
    def _load_dqparser(self, instrument):
        ...
        dqdict = self.settings.get('dqdict', _def_dqdict)
    ...
    def mark_dqs_cb(self, w, res_dict):
        ...
        for i, key in enumerate(res_dict):
            ...
            cur_col = self.imdqcolors[i % self.n_color]
...
if __doc__ is not None:
    __doc__ += generate_cfg_example('plugin_DQMask')
